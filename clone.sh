...
function main() {
    ...
    docker_run $CHIP_NAND_SCRIPTS_IMAGE mk_chip_image $NAND_CONFIG ubi        rootfs-$NAND_CONFIG.ubifs                   rootfs-$NAND_CONFIG.ubi
    ...
    case "$NAND_CONFIG" in
        hynix-mlc)
            HOWITZER_NAND_TYPE="Hynix_8G_MLC"
            ;;
        *)
            echo "ERROR: nand type "$NAND_CONFIG" not supported yet"
            exit 1
    esac

    docker_run $HOWITZER_IMAGE howitzer nand "$HOWITZER_NAND_TYPE" chp fel.chp chp fastboot.chp -f "${OUTPUT_CHP}"
